...
if(state.mode==="sites"){modePanelEl.className="panel interaction-panel sites-mode";const correctPool=getSiteChoicesForRoute(scenario.route);const otherRoutes=["IM","SQ","ID"].filter(r=>r!==scenario.route);const wrongRouteA=pickRandom(getSiteChoicesForRoute(otherRoutes[0]));const wrongRouteB=pickRandom(getSiteChoicesForRoute(otherRoutes[1]));const correctSite=pickRandom(correctPool);const distractors=[wrongRouteA,wrongRouteB,pickRandom(nonInjectionSites)];const options=shuffleArray([{kind:"route-site",route:scenario.route,name:correctSite.name,meta:correctSite.meta,correct:true,image:correctSite.image},{kind:"route-site",route:otherRoutes[0],name:distractors[0].name,meta:distractors[0].meta,correct:false,image:distractors[0].image},{kind:"route-site",route:otherRoutes[1],name:distractors[1].name,meta:distractors[1].meta,correct:false,image:distractors[1].image},{kind:"non-site",route:"NONE",name:distractors[2].name,meta:distractors[2].meta,correct:false,image:distractors[2].image}]);modePanelEl.innerHTML=`<h3 class="section-title">Choose the correct injection site</h3><div class="mini-card site-prompt-card" style="margin-bottom:14px;"><div class="route-icon ${scenario.route.toLowerCase()}">${guide.icon}</div><span class="mini-title">Prompt</span><p>Choose the best injection site for this prescription. Only one answer is appropriate.</p></div><div class="choice-grid two site-answer-grid">${options.map((opt)=>`<button class="choice-btn site-answer-option" data-correct="${opt.correct?"yes":"no"}" data-answer-name="${escapeHtml(opt.name)}" data-answer-route="${opt.route}"><div class="site-answer-art">${renderSiteOptionArt(opt)}</div><span class="choice-title">${opt.name}</span><span class="choice-sub">${opt.meta}</span></button>`).join("")}</div>`;document.querySelectorAll(".site-answer-option").forEach(btn=>btn.addEventListener("click",()=>{document.querySelectorAll(".site-answer-option").forEach(x=>x.classList.remove("selected-correct","selected-wrong"));if(btn.dataset.correct==="yes"){btn.classList.add("selected-correct");correctAdvance("Correct injection site selected.",`${btn.dataset.answerName} is an appropriate ${scenario.route} site for this prescription.`)}else{btn.classList.add("selected-wrong");showBad("Not quite.",`${correctSite.name} is the best answer here. It matches the ${scenario.route} route.`)}}));return}
...
const siteChoiceLibrary={
  IM:[
    {name:"Deltoid",meta:"Upper arm muscle",image:"assets/sites/deltoid.png"},
    {name:"Ventrogluteal",meta:"Hip area",image:"assets/sites/ventrogluteal.png"},
    {name:"Vastus Lateralis",meta:"Outer thigh muscle",image:"assets/sites/vastus_lateralis.png"},
    {name:"Dorsogluteal",meta:"Upper outer buttock",image:"assets/sites/dorsogluteal.png"}
  ],
  SQ:[
    {name:"Lower Abdomen",meta:"2 inches from navel",image:"assets/sites/abdomen.png"},
    {name:"Back of Upper Arm",meta:"Fat tissue area",image:"assets/sites/upper_arm_back.png"},
    {name:"Top of Thigh",meta:"Fat tissue area",image:"assets/sites/thigh_sq.png"}
  ],
  ID:[
    {name:"Forearm",meta:"Most common for PPD",image:"assets/sites/forearm.png"},
    {name:"Upper Back",meta:"Between shoulder blades",image:"assets/sites/upper_back.png"}
  ]
};
const nonInjectionSites=[
  {name:"Earlobe",meta:"Anatomical distractor",image:"assets/sites/earlobe.png"},
  {name:"Toes",meta:"Anatomical distractor",image:"assets/sites/toes.png"},
  {name:"Nose",meta:"Anatomical distractor",image:"assets/sites/nose.png"}
];
function getSiteChoicesForRoute(route){return siteChoiceLibrary[route]||[]}
function pickRandom(arr){return arr[Math.floor(Math.random()*arr.length)]}
function renderSiteOptionArt(option){
  const safeName=escapeHtml(option.name);
  const safeMeta=escapeHtml(option.meta||"");
  const safeImage=escapeHtml(option.image||"");
  return `<img class="site-option-image" src="${safeImage}" alt="${safeName} - ${safeMeta}" loading="lazy" />`;
}
