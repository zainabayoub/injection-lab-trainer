...
function render(){const scenario=state.questionSet[state.index]||scenarios[state.index],guide=routeGuides[scenario.route],syringe=syringeLibrary.find(s=>s.id===scenario.syringeType),total=state.questionSet.length||scenarios.length;currentModeEl.textContent=modeLabels[state.mode];progressTextEl.textContent=`${state.index+1} / ${total}`;scoreTextEl.textContent=state.score;progressFillEl.style.width=`${((state.index+1)/total)*100}%`;orderTextEl.textContent=scenario.order;medicationTextEl.textContent=scenario.medication;strengthTextEl.textContent=scenario.strength;bestSyringeTextEl.textContent=syringeDisplayName(syringe.id);if(bestSyringeRowEl)bestSyringeRowEl.style.display=state.mode==="tray"?"none":"";routePillEl.textContent=scenario.route;siteFamilyTextEl.textContent=`${scenario.route} Site Family`;routeCueTextEl.textContent=guide.cue;angleTextEl.textContent=guide.angle;lengthTextEl.textContent=guide.length;siteMapTextEl.textContent=guide.siteMap;siteDiagramEl.innerHTML=guide.sites.map(site=>`<div class="site-chip"><div><div class="site-name">${site.name}</div><div class="site-meta">${site.meta}</div></div><div class="soft-label">${scenario.route}</div></div>`).join("");gameGridEl&&gameGridEl.classList.toggle("tray-horizontal",state.mode==="tray");feedbackBoxEl.classList.toggle("tray-feedback",state.mode==="tray");clearFeedback();state.selectedSyringe=null;state.selectedGauge=null;state.readyForNext=false;renderModePanel(scenario,syringe,guide)}
