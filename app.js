...
const state={mode:"tray",index:0,score:0,selectedSyringe:null,selectedGauge:null};
...
function startMode(mode){state.mode=mode;state.index=0;state.score=0;state.selectedSyringe=null;state.selectedGauge=null;showScreen("game");render()}
...
function correctAdvance(message,explanation){state.score+=1;scoreTextEl.textContent=state.score;showGood(message,explanation);setTimeout(()=>{state.index+=1;if(state.index>=scenarios.length){showResults()}else{render()}},1100)}
function showResults(){showScreen("results");const total=scenarios.length,pct=Math.round(state.score/total*100);let rank="Beginner Injector";if(pct>=90)rank="Clinical Pro";else if(pct>=75)rank="Safe Injector";else if(pct>=60)rank="Emerging Injector";resultsTitleEl.textContent=rank;resultsBodyEl.textContent=`You scored ${state.score} out of ${total} in ${modeLabels[state.mode]}.`}
function showGood(message,explanation){feedbackBoxEl.innerHTML=`<div class="feedback good"><span class="feedback-title">Correct</span>${message}<span class="explain">${explanation||""}</span></div>`}
